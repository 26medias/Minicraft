...
**Architecture:** A new first phase in `LiquidScheduler.tick` (`applySpongeStep`) that BFS-absorbs from any dry sponge adjacent to a snapshot cell; a fix so orphan flow identified by drain never re-spreads in the same tick (a sponge hole next to an orphan puddle otherwise flickers forever); an exception in `decayFrontier` so sponge-adjacent liquid stays in the frontier; a DEV-only `window.__mc` debug hook so the manual check has an oracle; a shared `markDirty`/`flushDirty` pair so every scheduler write also dirties the neighbour chunk at chunk edges. No new block rows, no save-format change.
...
Extend the existing import: `import { BLOCK_BY_NAME, AIR, LAVA, OBSIDIAN, WATER } from '../data/blocks.data';` (keep `water`/`stone` consts as they are). `indexOf` is already imported mid-file (line ~82); do NOT add a second import, it is a TS2300 duplicate and breaks the build.
...
- [ ] **Step 2: Run to verify they fail** — first confirm a clean tree: `git status --porcelain` must be empty apart from your test edit (reviewers have left probe files in `src/game/` before; delete any `zz-*.test.ts`, `*probe*` files and revert stray edits to tracked files).
...
Also fix the mis-indented `this.changed = true;` lines in the three write loops while you are there (they are one tab short).

- [ ] **Step 6: World-boundary keys.** `markDirty` emits neighbour keys unconditionally, including `cx = -1` for a cell at world x=0. Gate 2 verified that `GameLoop.flushDirtyChunks` drops out-of-world keys via `chunkInWorld` and `ensureChunk`s unmounted in-world ones (generating and meshing them). Do NOT add a `getChunk` guard: it would drop legitimate neighbour remeshes. Add this test to the dirty-reporting describe block and run it green:
...
The loop side (`flushDirtyChunks` skipping `-1`) is covered by the existing `chunkInWorld` check; state that in the commit body.
...
(Test 8 is in Task 4; numbering follows the spec.)

- [ ] **Step 2: Run to verify they fail**

Run: `npx vitest run src/game/liquid-scheduler.test.ts -t sponge`
Expected: 1, 2, 3, 3b, 4, 5, 7, 9, 11, 12, 13 fail (sponge never turns wet, liquid remains). 6 and 10 pass trivially now; that is expected, they guard against regressions in Step 3.
...
- [ ] **Step 5: Prove the instruments bite.** One at a time, make the change, run the test named, confirm red, revert:
  ...
  - Replace the hop cap with a taxicab check (delete the `if (n.hops >= SPONGE_RADIUS) continue;` line and skip neighbours where `Math.abs(nx-sx)+Math.abs(ny-sy)+Math.abs(nz-sz) > SPONGE_RADIUS`) → test 3 red ((263,262) absorbed). Verified at gate 2; test 3b stays green under this mutation, which is expected (it pins wall leakage, not the metric).
...
### Task 3: Orphan flow never re-spreads

**Why:** drain peels only the outermost ring of an orphan puddle per tick and relies on AIR appearing next to inner orphan cells only through that peeling. A sponge hole breaks that: inner orphan cells (distance 1..3) next to the hole re-spread into it every tick while drain peels the ring back, so the puddle never settles (measured 12,8,10,8,10,… liquid cells forever, `changed` true every tick, remesh and autosave at 2 Hz). Fix: spread skips any cell drain classified as orphan this tick.

**Files:**
- Modify: `src/game/liquid-scheduler.ts` (`applyDrainStep`, `applySpreadStep`)
- Test: `src/game/liquid-scheduler.test.ts`

**Interfaces:**
- Produces (private): `orphansThisTick: Set<string>` populated by `applyDrainStep`, read by `applySpreadStep`.
...
Run: `npx vitest run src/game/liquid-scheduler.test.ts -t "settles to dry"`
Expected: FAIL, `liquidCount()` is 8 or 10 (the flicker), never 0.

- [ ] **Step 3: Implement.** In the class fields add `private orphansThisTick = new Set<string>();`. In `applyDrainStep`: first line of the body `this.orphansThisTick.clear();`. After step 4 (the `orphansByType` partition) and before step 5, add:

```ts
		for (const [, arr] of orphansByType) for (const o of arr) this.orphansThisTick.add(`${o.x},${o.y},${o.z}`);
```

In `applySpreadStep`, right after `if (!isLiquid(here)) continue;`:
...
- [ ] **Step 4: Run green, whole suite, build**

Run: `npx vitest run && npm run build`
Expected: test 14 passes; the existing drain-cascade tests still pass (an orphan ring still peels one per tick; it just no longer spreads).

- [ ] **Step 5: Commit**

```bash
git add src/game/liquid-scheduler.ts src/game/liquid-scheduler.test.ts
git commit -m "fix(liquids): orphan flow never re-spreads, so a sponge hole beside a draining puddle settles"
```

---

### Task 4: Frontier decay keeps sponge-adjacent liquid
...
Prove it bites: temporarily change `y + 1` in the exception to `y + 2`, run `-t "8b"`, expect FAIL (the cell at 262 has no AIR neighbour and no sponge on its 6 sides under the mutated rule, so it decays and the sponge stays dry); restore, expect PASS. Likewise test 8 must go red if the whole `touchesSponge` block is removed. Note both red runs in the commit body.
...
Also change the sentence "Each tick runs three phases — drain, spread, reaction" (line ~83) to "Each tick runs a sponge phase, then drain, spread, reaction", and in "### Phase 2 — spread" add: "Cells that drain classified as orphan this tick never spread; otherwise inner orphan cells next to a hole (a sponge's, or TNT's) refill it while drain peels the rim, and the puddle never settles."
...
Update the "Code map" bullet for `liquid-scheduler.ts` to mention the sponge phase, `orphansThisTick`, and `markDirty`. Update the header comment of `src/game/liquid-scheduler.ts` ("Three-phase liquid scheduler") to describe phase 0 (sponge) and the orphan rule; stage that file in this commit too.
...
In the README paragraph on line ~9 ("Gameplay toys on top of place/mine: TNT ..., and water/lava with simple block-by-block flow.") append ", and a sponge that soaks them back up". In the Tech Stack "Vitest" line, replace the hard-coded "450 unit tests" with "unit tests" (the number drifts every feature; stop stating it) and extend the scheduler list "(fall rule, sideways spread, frontier decay)" with ", sponge absorption".

- [ ] **Step 3: Commit**

```bash
git add docs/liquids.md README.md src/game/liquid-scheduler.ts
git commit -m "docs: sponge behaviour in liquids.md, README, scheduler header"
```

---

### Task 6: Debug hook and manual check at localhost

**Files:**
- Modify: `src/main.ts` (right after `loop.start();`, ~line 389)

**Why a code change:** nothing in `main.ts` is exposed on `window`, so a headless tester can only screenshot a 16 px yellow-vs-olive texture. That is a test that cannot go red. A DEV-only hook gives `world.getBlock` as the oracle and is tree-shaken from the production build.

- [ ] **Step 1: Add the hook** after `loop.start();`:
...
Run `npm run build` and confirm `grep -c "__mc" dist/assets/*.js` prints 0 for every file.

- [ ] **Step 2: Commit**

```bash
git add src/main.ts
git commit -m "chore(dev): expose world/player/loop on window.__mc in dev builds"
```

- [ ] **Step 3: Manual check.** Use a local-only world at `localhost:5173`, never `noah.leap-forward.ca`. `curl -s localhost:5173 >/dev/null || (npm run dev &)`. Open the world with the Playwright MCP tools. Pointer lock in headless: run in the page

```js
Object.defineProperty(Document.prototype, 'pointerLockElement', { get: () => document.querySelector('canvas') });
document.dispatchEvent(new Event('pointerlockchange'));
```

(verify first that mousemove then rotates the camera; if a play-time lock overlay is showing, choose "Start fresh"). Ids: sponge 315, wet sponge 363, water 17, lava 18, AIR 0. Read blocks with `window.__mc.world.getBlock(x,y,z)` and the player position from `window.__mc.player`. Mining a sponge is a held left click of about 0.3 s: mousedown, wait 400 ms, mouseup.

1. Dig a 3×3×2 pool, fill it with water, place a sponge on the rim (right-click a rim block face). Within 0.5 s every former water cell reads 0 and the sponge cell reads 363. Mine the sponge: the cells still read 0.
2. Same with lava: cells read 0 and, visually, the glow is gone (screenshot before/after).
3. Swim to the sea floor and right-click the floor to place a sponge into the water. A hole opens (cells around it read 0), then over the next seconds water falls back in (cells read 17 again). At the surface, place one on a sand block at the shoreline: a crater of 0s remains around the 363 cell after 10 s.
4. A pool spanning a chunk boundary (x or z a multiple of 16), sponge from one side, camera inside the hole looking at the far wall: screenshot; no see-through wall. This one is visual only.
5. Report pass/fail per step with the getBlock readings and screenshots in the scratchpad. No commit.
