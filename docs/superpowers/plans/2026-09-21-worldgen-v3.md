...
```ts
		maxWorkers: 2,
```
inside the existing `test: { … }` block (vitest 4.1.5: `maxWorkers` is the option that exists and typechecks; `poolOptions.*` fails with TS2769). State in the commit that this exists because of `src/engine/world/v3/**`.
...
function fullMap(seed: number) {
	...
	for (let cz = 0; cz < 32; cz++) for (let cx = 0; cx < 32; cx++) { const c = new Chunk(cx, cz, 256); generateChunkV3(c, seed); m.set(cz * 32 + cx, c.blocks); if (cx > 0 && cz > 0 && cx < 31 && cz < 31 && W.replays < 37) replaysBad++; // ≥ 37: Task 4's 36 + pools; Task 5 adds 9 → 46, asserted exactly in Task 7 (gate 2 closure: never assert equality on a counter a later task adds to) instMax = Math.max(instMax, W.instances); }
...
describe('v3 features', () => {
	...
	it('§11.14 pools: every placed pool voxel has 4 solid-or-liquid sides and a solid below; ore voxels replaced only stone/deepslate (checked via a no-feature comparison)', () => {
		...
		for (let cz = 14; cz <= 17; cz++) for (let cx = 14; cx <= 17; cx++) { const log: number[] = []; const c = new Chunk(cx, cz, 256); generateChunkV3(c, 2, { poolLog: log });  // poolLog rides on the Capture parameter (Task 3)
			expect(log.length).toBeGreaterThan(0); // non-vacuity: at least one pool voxel in this 4×4 block (seed 2 has 64 pool instances here)
			for (const i of log) { const yy = (i / 256) | 0, lz = ((i % 256) / 16) | 0, lx = i % 16; expect(isLiquidId(c.get(lx, yy, lz))).toBe(true);
...
Run: `npx vitest run src/engine/world/v3 && npx tsc --noEmit && npm test` — note: `src/engine/world/v3` includes Task 4's `features.test.ts` when both branches are merged; its replay clause is `≥ 37`, so it stays green with trees adding 9 (the merge step re-runs it).
...
- `T14.poolVoxelBad`: record every voxel each `pool()` call writes (`ctx.poolLog?: number[]` is already on `Ctx` since Task 3 and `pool()` already pushes into it since Task 4 — no file outside Task 7's list changes) and assert for EVERY written liquid voxel: 4 side neighbours solid-or-liquid and the voxel below solid. (Gate 2: the prototype's "found voxels" loop looks at one voxel per pool.)
...
| R-N4: no mutant executed; the checker must scan every voxel (no x/z sampling) | `checkMap(1, { bandShift: { coal: 8 } })` asserted red; "scan every voxel" stated |
...
| R-U5: `T14.poolVoxelBad` under-specified | Every voxel a pool writes (`ctx.poolLog`, field on `Ctx` from Task 3, pushed by Task 4's `pool()`, asserted in Tasks 4 and 7) |
...
**Gate 2 closure (rigour, run verbatim).** B1 Task 4 replay equality → `≥ 37` (Task 7 owns `== 46`); B2 pool test asserted over `capture.poolLog` (Ctx field from Task 3, pushed by Task 4's `pool()`), never a column scan (the lava sea sits in the scan window); B3 `maxWorkers: 2` (vitest 4.x has no `poolOptions`); kid.ts port drops `spawn.ms`; coalshift evidence number corrected (≈ 1 800); placeholder upper bounds replaced with real ones.
