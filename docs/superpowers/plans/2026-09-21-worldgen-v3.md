...
**Tech Stack:** TypeScript strict, Vite, vitest, `simplex-noise` 4.x (`createNoise2D/3D`), `alea` (noise-field seeding only), Three.js untouched. Playwright MCP for Task 10.
...
- Every block id comes from `BLOCK_BY_NAME[name].id` (`src/data/blocks.data.ts`); the 60 names in `v3/blocks.ts` all exist in the frozen catalog (verified 2026-09-21, none retired). Never hardcode numeric ids.
- Tests that generate whole maps are slow (≈ 2 s per 1024-chunk map to generate, ≈ 6 s with checks). Per-commit vitest files may generate at most **two** full maps (seeds 1 and 2); the 8-seed CI sweep and the 100-seed kid suite are on-demand scripts (Task 7), stated in the test file header.
- Commit after every task with explicit paths (`git add <paths>`, never `git add -A`). Trailer on every commit:
  ...
  ```
- Browser verification (Task 10) runs ONLY against `http://localhost:5173` started with `VITE_MINICRAFT_API_URL=http://127.0.0.1:9099 npm run dev` (the env var on the command line beats `.env.local`, which points at the **production** save API). Never edit `.env.local`; never touch `https://noah.leap-forward.ca`.
...
| 3 Lattice, fill, surface (stages 2–4) + chunk tests | 2 | 4 (lists only), 5 (lists only) |
| 4 Features: instance lists + writers + pools + decoration | 2 (lists) / 3 (writers) | 5 |
| 5 Trees: species, `treesOf`, templates | 2 (lists) / 3 (writers) | 4 |
| 6 Spawn + `main.ts` hand-off + menu message | 3, 4, 5 | 7 |
| 7 CI instrument: map checker vitest + on-demand scripts | 3, 4, 5 | 6 |
| 8 Hash bootstrap + docs | 6, 7 | — |
| 10 Browser verification | 8 | — |

Tasks 4 and 5 both import `generate.ts`'s `Ctx`/`put` helpers from Task 3 and add their own files + tests; they touch disjoint files. Task 3 leaves stubs `features`/`trees` hooks off (see Task 3 step 3: the stage-5/6 calls are added by Tasks 4 and 5 in `generate.ts` — **these two edits to `generate.ts` are the only overlap; Task 5 rebases on Task 4's merge or vice versa; the edits are 6 lines each and do not conflict textually if made at the marked comments**).

**Execution note.** Every task's exit criterion is the FULL `npm test` plus `npx tsc --noEmit`, so two tasks cannot share a working tree. Parallel tasks run in separate `git worktree`s on their own branches (`task/wg-N`, cut from `worldgen` at the point their dependencies have merged). A merge step (parent) merges each branch into `worldgen` in dependency order and re-runs `npm test && npx tsc --noEmit` after every merge. Worktrees cut from an old commit must `git reset --hard worldgen` first (this happened in the v2 run). Root `package-lock.json` is out of sync with `package.json` (`npm ci` fails on `@emnapi/*`): symlink the main checkout's `node_modules` into a worktree instead of installing; never touch the lock file.
...
- Modify: `src/engine/world/generation.test.ts` (the two assertions the spec names, lines 213–216)
...
Also edit `src/engine/world/generation.test.ts` lines 213–216 to:
```ts
...
Expected: all green; `EXPECTED_HASH` and `EXPECTED_HASH_V2` tests unchanged and green (v1/v2 untouched). Note: `World.create(seed)` now yields `genVersion 3` — any existing test that asserted `World.create(...).genVersion === 2` must be updated to 3 (grep `genVersion).toBe(2)` in `src/engine/world/world.test.ts` and `src/main`-adjacent tests; update the number only).

- [ ] **Step 5: Commit**

```bash
git add src/engine/world/v3/prng.ts src/engine/world/v3/prng.test.ts src/engine/world/v3/blocks.ts src/engine/world/v3/blocks.test.ts src/engine/world/v3/fields.ts src/engine/world/v3/fields.test.ts src/engine/world/v3/generate.ts src/engine/world/generation.ts src/engine/world/generation.test.ts
git commit -m "feat(worldgen): v3 scaffolding — hash streams, block table, field cache, dispatcher branch"
```
(plus any `world.test.ts` genVersion update in the same commit).
...
describe('v3 columns', () => {
	...
	it('ravine zones are absent within 128 of the centre and the threshold ramps to 0.35 by 176 (§6.1)', () => {
		...
		for (let i = 0; i < 20000; i++) { const x = (i * 97) % 512, z = (i * 193) % 512; const c = column(2, x, z);
			...
			if (c.ravW > 0) { const rgT = d >= 176 ? 0.35 : 0.4; expect(f.RG(x / 300, z / 300)).toBeGreaterThan(rgT - 1e-9); expect(c.h).toBeGreaterThan(SEA + 8); expect(c.amp).toBeLessThan(6); }
		}
	});
...
- Produces: `generateChunkV3(chunk, seed, capture?)`, `Ctx`, `Setter`, `put`, `W`, `Capture`, `PAD = 4`, `PAD_HI = 5`, `PW = 25`, `LAT = 4`. Two marked insertion points for Task 4 (`// STAGE 5 (Task 4)`) and Task 5 (`// STAGE 6 (Task 5)`).
...
const H = 256;
/** Generate an (x0..x1) × (z0..z1) block of chunks; `get` returns -1 outside it. */
...
export function generateChunkV3(chunk: Chunk, seed: number, capture?: Capture): void {
	...
	// STAGE 5 (Task 4): underground features + pools + decoration — `stage5(ctx, seed, cx, cz, col, wn, caveCeil, kind, yTop, f)`
	// STAGE 6 (Task 5): trees — `stage6(ctx, seed, cx, cz)`
	void ctx; void cx; void cz;
...
```bash
git add src/engine/world/v3/generate.ts src/engine/world/v3/generate.test.ts
git commit -m "feat(worldgen): v3 stages 2–4 — density lattice, caves, ravines, water, surface rules"
```
...
**Files:**
- Create: `src/engine/world/v3/features.ts`, `src/engine/world/v3/features.test.ts`
- Modify: `src/engine/world/v3/generate.ts` — replace the `// STAGE 5 (Task 4)` comment with the call shown in step 3 (6 lines) and add the import.
...
- Produces: everything listed for `features.ts` in Shared definitions, plus `stage5(ctx, seed, cx, cz, col, wn, caveCeil, kind, yTop, f)` exported from `features.ts` and called from `generate.ts`.
...
import { oresOf, blobsOf, pocketsOf, geodesOf, poolsOf } from './features';
...
function fullMap(seed: number) {
	...
	for (let cz = 0; cz < 32; cz++) for (let cx = 0; cx < 32; cx++) { const c = new Chunk(cx, cz, 256); generateChunkV3(c, seed); m.set(cz * 32 + cx, c.blocks); if (cx > 0 && cz > 0 && cx < 31 && cz < 31 && W.replays !== 46) replaysBad++; instMax = Math.max(instMax, W.instances); }
...
describe('v3 features', () => {
	...
	it('§11.14 pools: every placed pool voxel has 4 solid-or-liquid sides and a solid below; ore voxels replaced only stone/deepslate (checked via a no-feature comparison)', () => {
		...
		// ores replace only stone/deepslate: an ore voxel's 6 neighbours that are not ore/blob/pocket must be stone-ish or air-cave... simplest exact check: ore never sits where `kind` was not solid — covered by §11.5/§11.6 (no ore in cave-air) via the region test in Task 7. Here: no ore at y 0 (bedrock) and none above 230.
...
export function stage5(ctx: Ctx, seed: number, cx: number, cz: number, col: (lx: number, lz: number) => Col, wn: Uint8Array, caveCeil: Float32Array, kind: Uint8Array, yTop: number, f: Fields): void {
	...
	for (let ncz = cz - 1; ncz <= cz + 1; ncz++) for (let ncx = cx - 1; ncx <= cx + 1; ncx++) {
		...
		W.replays++; for (const o of oresOf(seed, ncx, ncz)) {
			...
			W.instances++; vein(ctx, o, (at) => (isStoneId(at) ? (at === V3.deepslate ? ORE_DEEPSLATE.get(o.ore.id)! : o.ore.id) : -1));
		}
...
```bash
git add src/engine/world/v3/features.ts src/engine/world/v3/features.test.ts src/engine/world/v3/generate.ts
...
- Modify: `src/engine/world/v3/generate.ts` — replace the `// STAGE 6 (Task 5)` comment with `stage6(ctx, seed, cx, cz);` and add `import { stage6 } from './trees';`.
...
```
In `generate.ts`: `import { stage6 } from './trees';` and replace `// STAGE 6 (Task 5)` with `stage6(ctx, seed, cx, cz);`.
...
```bash
git add src/engine/world/v3/trees.ts src/engine/world/v3/trees.test.ts src/engine/world/v3/generate.ts
git commit -m "feat(worldgen): v3 stage 6 — five tree species with border-safe replay"
```

---

### Task 6: Spawn — `spawnV3`, in-memory hand-off in `main.ts`, "Building your world…" with a frame yield
...
- Create: `scripts/worldgen-kid.ts` (on-demand 100-seed suite, port of `kid.ts` against the engine modules; not run by vitest)
...
- Produces: `spawnV3(seed): SpawnInfo`; `MainMenu.showBuilding(text)`; `main.ts` new-world flow: `menu.showBuilding('Building your world…'); await new Promise(requestAnimationFrame); const s = spawnV3(seed); ... findSafeSpawn(world, [s.x + 0.5, world.height - 1, s.z + 0.5])`.
...
describe('spawnV3 (§9, §11.11)', () => {
	...
	it('strict spawn safety on the CI seeds: top block is exactly h, solid, not liquid/snow/ice; air at h+1, h+2; h ≥ 122; no log/leaf within Chebyshev 3 and 12 up; buildable ≥ 40 %; no ravine channel within 64; pass 1 ⇒ h ≥ 130', () => {
		for (const seed of CI) {
			...
			for (let dz = -64; dz <= 64; dz += 2) for (let dx = -64; dx <= 64; dx += 2) { const x = s.x + dx, z = s.z + dz; if (x < 0 || z < 0 || x > 511 || z > 511) continue; if (column(seed, x, z).ravW > 0) expect(inRavineChannel(seed, x, z), `ravine channel within 64, seed ${seed}`).toBe(false); }
		}
	}, 180_000);
});
```
...
```ts
		...
		// message and yield one frame so it paints before the synchronous search.
...
Implements spec §11 as a runnable instrument: the exact counters and statistical bounds of §11.2–11.10, 11.13–11.16 over whole maps. Per-commit vitest runs seeds **1 and 2** (≈ 25 s); the 8-seed sweep and the 100-seed derivation are `npm run worldgen:check`. The mutant list (§11) is documented next to each assertion as the "how to prove red" note; mutants are NOT kept in engine code.

**Files:**
- Create: `src/engine/world/v3/mapcheck.ts` — pure checker `checkMap(seed): { exact: Record<string, number>; stats: Record<string, number> }` (port of `check.ts`, using engine modules and the `Capture` kinds)
...
- Modify: `package.json` scripts: `"worldgen:check": "tsx scripts/worldgen-check.ts"`
...
/** Per-commit instrument: seeds 1 and 2 only (≈ 25 s). The 8 CI seeds and the 100-seed derivation: `npm run worldgen:check 1,2,3,5,8,13,21,34`. Bounds are the §11 values (100-seed derivation + margin rule); mutants named in §11 turn each red. */
...
describe('worldgen v3 map instrument (§11)', () => {
	for (const seed of [1, 2]) it(`seed ${seed}: all exact counters 0 and every statistical bound holds`, () => {
		...
		for (const [k, v] of Object.entries(exact)) expect(v, k).toBe(0);
...
- Add `T14.poolVoxelBad` (every liquid voxel at a pool's floor has 4 solid-or-liquid sides and solid below — check the `pools.perMap` loop's found voxels) and `ravine.big`: 8-connected components of channel columns (`inRavineChannel`) with size ≥ 40, counted per map.
...
Run: `npx vitest run src/engine/world/v3/mapcheck.test.ts && npx tsc --noEmit && npm test && npm run worldgen:check 1,2,3,5,8,13,21,34`
Expected: vitest green (≈ 25–40 s); the script prints every exact counter `ok … = 0` on all 8 CI seeds and stats inside the §11 bounds (this is the §11 CI-seed claim; record the script's output in the commit message body).
...
- Create: `scripts/worldgen-hash.ts` (prints the four FNV-1a-32 values for the engine AND for the reference prototype)

**Interfaces:**
- Consumes: `hashBytes` (already in `generation.test.ts`, FNV-1a-32 over Uint16 elements); `generateChunk(chunk, seed, 3)`; the reference prototype's `generateChunkV3(blocks, seed, cx, cz)` (imported from `../docs/superpowers/reference/worldgen-v3-prototype/proto.ts` by the script only).
- Produces: `EXPECTED_HASH_V3: readonly [number, number, number, number]` for chunks (0,0), (16,16), (31,31), (5,27) of seed 12345.
...
Expected: 1 skipped (reason visible), rest green.

- [ ] **Step 3: Bootstrap**

`scripts/worldgen-hash.ts`:
```ts
// §10 bootstrap helper. Prints FNV-1a-32 (over Uint16 elements) of chunks (0,0),(16,16),(31,31),(5,27) at seed 12345 for the ENGINE and for the reference PROTOTYPE.
import { Chunk } from '../src/engine/world/chunk';
import { generateChunk } from '../src/engine/world/generation';
import { generateChunkV3 as protoGen } from '../docs/superpowers/reference/worldgen-v3-prototype/proto';
...
const proto = chunks.map(([cx, cz]) => { const b = new Uint16Array(16 * 256 * 16); protoGen(b, 12345, cx, cz); return hashBytes(b); });
console.log('engine   ', JSON.stringify(eng)); console.log('prototype', JSON.stringify(proto)); console.log(eng.every((v, i) => v === proto[i]) ? 'AGREE' : 'DISAGREE — the implementation is wrong until proven otherwise (spec §10 step 4)');
```
Note: the prototype's `B` table hardcodes ids (`grass: 1, dirt: 2, stone: 3, … coal: 50 …`) that do **not** equal the catalog ids, so its raw hash will differ from the engine's by construction. To cross-check, the script must map: generate with the prototype, then remap each voxel id through `NAME[id]` → `BLOCK_BY_NAME[name].id` (prototype names: `grass`→`grass_block`, `snow`→`snow_block`, `coarse`→`coarse_dirt`, `moss`→`moss_block`, `dripstone`→`dripstone_block`, `amethyst`→`amethyst_block`, `budding`→`budding_amethyst`, `o_terra`→`orange_terracotta`, `y_terra`→`yellow_terracotta`, `w_terra`→`white_terracotta`, `r_terra`→`red_terracotta`, `br_terra`→`brown_terracotta`, `lg_terra`→`light_gray_terracotta`, `coal`→`coal_ore`, `d_coal`→`deepslate_coal_ore`, likewise for the other ores; `mossy_cobble`→`mossy_cobblestone`; the rest are identical names) before hashing. Implement that remap in the script (a `Record<string,string>` of the differing names, default identity).
Then: `./node_modules/.bin/tsx scripts/worldgen-hash.ts` in two separate processes; both must print identical engine values and `AGREE`. If `DISAGREE`: find the first differing voxel (add a diff loop to the script) and fix the engine port; never edit the prototype. Write the four values into `EXPECTED_HASH_V3` and into the spec §10 ("bootstrapped 2026-MM-DD: engine = prototype = [..]"), and one line in the prototype README.
...
```bash
git add src/engine/world/generation.test.ts scripts/worldgen-hash.ts docs/superpowers/specs/2026-09-21-worldgen-v3-design.md docs/specs.md README.md docs/superpowers/reference/worldgen-v3-prototype/README.md
...
### Task 10: Browser verification at localhost (no commit)
...
```bash
cd /home/julien/Projects/Minicraft && VITE_MINICRAFT_API_URL=http://127.0.0.1:9099 npm run dev > /tmp/wg-dev.log 2>&1 &
sleep 4 && curl -s -o /dev/null -w "%{http_code}\n" http://localhost:5173/
```
Expected: `200`. Then with Playwright: `page.route('http://127.0.0.1:9099/**', r => r.fulfill({ status: 404, body: '{}' }))` and `page.route(/a\.run\.app|leap-forward\.ca/, r => r.abort())` **before** `browser_navigate` to `http://localhost:5173/`. Clear localStorage on first load (`localStorage.clear()` via `browser_evaluate`) so the world list is empty.

- [ ] **Step 2: New world → hill with a view (Task 6, §9)**

Click "New World", name `wg-check`, seed `3`, create. Assert via `browser_evaluate`: `window.__mc.world.genVersion === 3`, `window.__mc.world.height === 256`, `__mc.player.position[1] >= 123` (spawn h ≥ 122 + 1), the block under the player is `grass_block`/`podzol`/`coarse_dirt`/`red_sand`/`sand` (not water/snow/ice), and the message "Building your world…" was painted (check `performance.getEntriesByType('paint')` length grew, or take a screenshot immediately after clicking — the card text must be visible for at least one frame; if the search is < 50 ms it may not be, note it). Screenshot `spawn-view.png`.

- [ ] **Step 3: Walk to a cave mouth and dig (Tasks 3–5)**

Using `spawnV3(3)` from a tsx one-liner (`./node_modules/.bin/tsx -e "import {spawnV3} from './src/engine/world/v3/spawn'; console.log(spawnV3(3))"`) and `scripts/worldgen-kid.ts 3 1` output, find the nearest cave mouth coordinates; teleport there with `__mc.player.position = [x+0.5, y, z+0.5]` and screenshot `cave-mouth.png` — expect a sloping entrance with a roofed tunnel, no 30-block pit. Fly down (F) into the tunnel: screenshot `cave-inside.png` — expect ore visible in walls within 30 blocks (coal/iron/copper), moss or dripstone somewhere below y 112.
...
**Type consistency.** `Ctx`/`Setter`/`put`/`W`/`Capture`/`PAD`/`PW` are defined in Task 3's `generate.ts` and imported by Tasks 4 and 5 (`features.ts` also imports `PAD, PW` for the `wn` index); `Col` is exported by Task 2; `stage5`'s signature `(ctx, seed, cx, cz, col, wn, caveCeil, kind, yTop, f)` matches the call in `generate.ts`; `stage6(ctx, seed, cx, cz)`; `spawnV3` returns `SpawnInfo` with `pass` 1–5; `treeTemplate` returns `[x, y, z, id][]`; `checkMap` returns `{ exact, stats }` with the keys listed in Task 7's Interfaces and asserted in its test. `V3` keys are the 60 catalog names (Task 1), used as `V3.grass_block`, `V3.snow_block`, `V3.coarse_dirt`, `V3.moss_block`, `V3.dripstone_block`, `V3.amethyst_block`, `V3.budding_amethyst`, `V3.smooth_basalt`, `V3.<family>_ore`, `V3.deepslate_<family>_ore`, `V3.<species>_log/_leaves` throughout — never the prototype's short names.
