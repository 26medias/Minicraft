...
- Produces:
  ```ts
  export function craftedBlockSlot(hotbar: BlockId[], selected: number, id: BlockId, inv: Inventory, mustMine: boolean): number;   // holding → first empty → first other slot that needs a count and is at 0 → selected
...
- [ ] **Step 2: Run; expect failure.** `npx vitest run src/game/craft-apply.test.ts` fails at import. That failure is structural. The real reds are mutants A1–A4 (table above), each run in the spike against the finished module:
  - A1 routes the crafted TNT through `autoHotbar`. In the unlimited-world test auto-hotbar is a no-op, so the TNT never reaches the bar.
...
puts crafted TNT in the slot holding it, else the first empty one, else a greyed 0-count slot, else
...
export function craftedBlockSlot(hotbar, selected, id, inv, mustMine): number;
export type CraftOutcome; export function applyCraft(player, recipe, mustMine, markDirty): CraftOutcome;
// src/ui/inventory.ts
