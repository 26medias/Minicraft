...
- [ ] **Step 2: Run them and see them fail.** Run `npx vitest run api/src/handlers.test.ts`. This was verified on a copy with only B2 applied: 7 fail.
...
- [ ] **Step 2: Run them and see them fail.** Run `npx vitest run src/persistence/dual.test.ts`. This was verified on 3114ce7: 7 fail (the 5 below plus the two old-bundle tests named after them).
...
- [ ] **Step 4: Run; expect pass.** Run `npm run build-atlas`. It must print `Wrote 461 tiles` on a tree whose atlas had 447 tiles (+6 derived faces, +8 icons). If Phase A–C added no block textures, 461 is exact. Then run `npx vitest run src/data/atlas-derive.test.ts && npx tsc --noEmit`. Expect 6 passed.
...
- `Expected 1-3 arguments, but got 4` for `inventory.setHotbar`;
...
Also, in A6, add a bench row `craft-r8-ocean`: an r8 blast whose sphere touches the nearest ocean at seed 3, so that the water rushing into a Mega crater is measured against the same 50 ms gate.
