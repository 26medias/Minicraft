...
**Interfaces:**
- Produces: `setArea(min: readonly [number, number, number], max: readonly [number, number, number], multi: boolean): void`, and `export const HIGHLIGHT_WHITE = 0xffffff, HIGHLIGHT_AREA = 0xff8c1a, AREA_BOX_PAD = 0.01`.
- Call order: `show()`, then `setArea()`, every frame. The box is shown only while the face group is visible. `hide()` hides both.
- The box is a `THREE.LineSegments` added to the scene beside the group, with `depthTest: false`, so the part of the area inside the wall stays visible.
...
- [ ] **Step 2: Run it.** `npx vitest run src/engine/render/face-highlight.test.ts`. Expected: the 3 new tests FAIL with `h.setArea is not a function` (compile-class). Each names its wrong implementation. The existing test `scene.children.filter(Group)).toHaveLength(1)` must stay green; the box is a `LineSegments`, not a `Group`.
