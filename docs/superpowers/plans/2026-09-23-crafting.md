...
4. **Pressing P with only the Hand owned.** A no-op: no timer reset, no highlight flicker, no dirty save.
...
- **Bench, A1.** Run against unmodified engine code in copy 1: `npx tsx scripts/perf-bench.ts --phases craft --reps 4 --port 5191`. Headed Chromium ran on `DISPLAY=:1`. The printed table is in A1 step 3.
...
- Test: the bench itself (`npm run perf:bench -- --phases craft`).
...
- [ ] **Step 3: Run the baseline on today's code. Expect FAIL.**
  ...
  Run `npm run perf:bench -- --phases craft --reps 4`. It needs a display, because it launches a headed Chromium. It exits 1, and at least **`TNT r3 corner` prints FAIL**. That red is what shows the gate can go red (spec §8).
...
- Test: `npm run perf:bench -- --phases craft --reps 4`.
...
- [ ] **Step 1: Run the bench. Expect PASS.** Run `npm run perf:bench -- --phases craft --reps 4`. It exits 0, and every crafting row prints `ok`. Every bulk column is a number ≤ 16, because the build now has a bulk lane.
...
is still thin (~48 ms): it is the one chunk's own synchronous re-mesh, the same as a single edit in
a dense chunk.
```
...
r3 blast went from 52 ms to 34 ms. The interior r3 row stays thin at
~48 ms (one chunk's own synchronous re-mesh).
...
1. **`TNT r3 interior` is thin: about 48 ms against the 40 ms margin.** It is the anchor's own synchronous re-mesh of a dense surface chunk, the same cost a single edit has. Batched removal cannot reduce it, and the spec's fallbacks are about Mega's radius, not this. The 16 break particles per batch are new and land in the same frame. If Noah's laptop is 20 % slower, this row could cross 50 ms. The fix would be outside this spec: cheaper meshing, or moving the anchor to the worker, which trades away the instant hole.
2. **The bench needs a display.** It launched a headed Chromium here on `DISPLAY=:1` and ran fine. On a headless box it cannot run, and it must not be faked.
...
- [ ] **Step 6: Bench (spec §8, §13 step 5).** Run `npm run perf:bench` in full. Phase A added its crafting rows (plain TNT r3, 5×5×5 area mine, one Mega TNT, a chain of 4 Mega TNT, each interior and at a chunk corner) to the default phase list.
...
Expected before the fix: the test passes once A4's implementation is in. To confirm it can go red, replace `world.setBlock` with `chunk.set` in `removeBlocks`: the cell stays AIR.
...
it('a block already on the hotbar twice is not added a third time (catches the "not on hotbar" check scanning only the selected slot)', () => {
	...
	expect(autoHotbar(hotbar, 0, { stone: 3 }, [stone], true)).toBeNull();
});
```

### RF4 → Task C10, extra step: P with only the Hand owned does nothing

In the P handler that C10 adds to `main.ts`, the first line after computing `next` is:

```ts
		if (next === player.tools.equipped) return; // only one pickaxe owned: no reset, no dirty save
```

Append to the `tools.ts` test file:
