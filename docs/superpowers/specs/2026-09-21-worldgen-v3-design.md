...
4. ravine (§6.1);
5. **open-sky pit cap** (implementation feedback, 2026-09-21): after rules 1–4, in every column that is not a ravine-carving column (§6.1), walk down from `yTop`; while only air has been seen (no solid, no water yet), any cave-air voxel at `y ≤ h − 24` becomes solid. Roofed cave-air is untouched, so sloping mouths keep their continuation; only vertical shafts open to the sky are plugged. Guarantees `h − terrain top ≤ 24` off ravine channels (11.15). Before this rule a 1–2-column tunnel shaft open to the sky could run 42 voxels deep (seed 2 at (16,16)); mouths, cave share and every other counter are unchanged by it (§12).
...
- **Reference hash bootstrap (executable):** (1) implement until 11.1–11.16 are green; (2) `generation.test.ts` hashes `blocks` of chunks **(0,0), (16,16), (31,31), (5,27)** of seed 12345 — fixed chunks, never the spawn chunk (spawn constants may be tuned without a version bump) — with the repo's existing **FNV-1a-32** `hashBytes`, which XORs each **Uint16 element** of `blocks` (not bytes) in index order — the prototype's ids are remapped to engine ids (`NAME[id]` → `BLOCK_BY_NAME[name].id`) before hashing, so both sides hash the identical element sequence; (3) run the vitest suite twice in separate processes; only when both agree commit the four values as constants, the commit message naming this section; (4) the prototype is committed as reference only (`docs/superpowers/reference/worldgen-v3-prototype/`, never imported by `src/`): at bootstrap time its four remapped FNV-1a-32 values are written into this section and the implementation's values are cross-checked against them. **The one rule on disagreement:** the engine port is presumed wrong; the executor hashes each stage's output (columns → lattice → fill → features → trees) engine vs prototype to find the first divergent stage; only if the divergence is a spec-vs-prototype conflict does the spec win — then the prototype is patched with a dated comment and the hash re-run; (5) thereafter any constant change is `genVersion 4`. No browser leg (no runner exists). Until step 3 the test is skipped with a reason.
  **Bootstrapped 2026-09-21:** engine = prototype = `[2020764513, 800740276, 2743801548, 161955245]` for chunks (0,0), (16,16), (31,31), (5,27) of seed 12345 (prototype ids remapped to catalog ids by `docs/superpowers/reference/worldgen-v3-prototype/hash.ts`); two separate processes agreed and both printed `AGREE`. Recorded in `src/engine/world/generation.test.ts` as `EXPECTED_HASH_V3`; never re-recorded.
  **Re-run 2026-09-21 (pre-ship) after the snow-line rule (§4 rule 3, commit 17917e1):** `hash.ts` in two separate processes printed `AGREE` with the same four values `[2020764513, 800740276, 2743801548, 161955245]` — no column of the four reference chunks (or their −4/+5 pads) changes its snow-line verdict between the old and new formulas (they are too low or too warm), so the hashes were not re-recorded. The reference hashes therefore do not exercise rule 3; its instrument is the seed-3 flank test in `src/engine/world/v3/columns.test.ts` (columns 326..336 × 192..202 snow-topped after generation) plus `T10.snowLineTopBad` in the map check.
...
15. **E/S shape (map):** isolated floating solids ≤ 60 (100-seed max 47 + margin; the mean + 4 sd rule gives 46, which CI seed 3 exceeds at 47, so this bound uses max + 25 % of range instead); every ravine-core column's floor ≥ `bottom − 1` (0); rim-to-floor depth ≤ 70 where present (41–66); **E pit depth:** for every land column not in a ravine channel, `h − terrain top ≤ 30` (terrain top = highest voxel that is not air, liquid, ice, log or leaf), i.e. `pit.over30 == 0`, and **S** `pit.maxDepth ≤ 24` per map (CI seeds 24/24/21/22/24/21/21/20; before §6 rule 5: 32/50/21/22/26/21/21/20 with 2 columns over 30; mutant `nopitcap` red on seeds 1 and 2); **at least one ravine with ≥ 40 8-connected channel columns per map on the CI seeds** (measured 1–5 such ravines, largest 92–599 columns, on all 8 CI seeds; of the fresh seeds 101–106, five pass and seed 101 does not — 37 channel columns, largest 12 — see §12).
