...
export const SEA_LEVEL_V1 = 28;
const DIRT_BAND = 3;

export function generateChunkV1(chunk: Chunk, seed: number): void {
	if (chunk.height !== 64) throw new RangeError('generator v1 needs a 64-high chunk');
	...
	for (let lz = 0; lz < CHUNK_SIZE_Z; lz++) {
		for (let lx = 0; lx < CHUNK_SIZE_X; lx++) {
			...
			for (let y = 0; y <= h; y++) {
				let id: number;
				if (y === h) id = h < SEA_LEVEL_V1 ? SAND : GRASS;
			...
			if (h < SEA_LEVEL_V1) {
				for (let y = h + 1; y <= SEA_LEVEL_V1; y++) {
