/**
 * Reference implementation for the equivalence tests. Never called by the game.
 *
 * Verbatim copy of the pre-perf `computeChunkShadows` (per-voxel Amanatides-Woo DDA
 * through `world.getChunk`) with its helpers, frozen so Tasks 2 (shadow DDA rewrite)
 * and 3 (mesher golden) can compare their output against today's caster byte for byte.
 */
...
function maxOpaqueYBrute(chunk: Chunk): number {
...
function neighbourhoodMaxOpaqueYBrute(world: World, chunk: Chunk): number {
	let m = -1;
	for (let dx = -1; dx <= 1; dx++)
		for (let dz = -1; dz <= 1; dz++) {
			const c = world.getChunk(chunk.cx + dx, chunk.cz + dz);
			if (c) m = Math.max(m, maxOpaqueYBrute(c));
		}
	return m;
}
...
export function computeChunkShadowsBrute(world: World, chunk: Chunk): void {
	...
	const skyFrom = neighbourhoodMaxOpaqueYBrute(world, chunk) + 1;
	...
	for (let y = 0; y < chunk.height; y++) {
		for (let z = 0; z < CHUNK_SIZE_Z; z++) {
			for (let x = 0; x < CHUNK_SIZE_X; x++) {
				...
				chunk.sunlit[idx] = rayHitsSolidInLoadedChunksBrute(
					world,
					baseX + x + 0.5,
					y + 0.5,
					baseZ + z + 0.5,
...
function rayHitsSolidInLoadedChunksBrute(
	world: World,
	ox: number,
	oy: number,
	oz: number,
